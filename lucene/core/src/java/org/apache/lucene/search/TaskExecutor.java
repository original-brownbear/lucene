...
public final class TaskExecutor {
  ...
  public <T> List<T> invokeAll(Collection<Callable<T>> callables) throws IOException {
          ...
          new FutureTask<>(
              () -> {
                try {
                  ...
                  taskId.set(futures.length); // stop all task execution right away
                  for (int j = id + 1; j < futures.length; j++) {
                    futures[j].cancel(false);
                  }
                  throw t;
                }
              }) {
    ...
    final Runnable work = () -> runTasks(taskId, futures);
    for (int j = 0; j < count - 1; j = Math.max(j + 1, taskId.get())) {
  ...
  private static <T> void runTasks(AtomicInteger taskId, RunnableFuture<T>[] futures) {
    int id;
